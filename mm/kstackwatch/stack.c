...
#define AVGC(cy, cnt) ((cnt) ? ((cy) / (cnt)) : 0ULL)
	...
	pr_info("exit (without watch):  %llu ns, %llu cycles (%llu samples)\n",
		...
		AVGC(sum.total_exit_without_watch_cycles,
		     sum.exit_without_watch_count),
		sum.exit_without_watch_count);
}
