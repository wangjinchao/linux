...
static int stack_config_parse(const char *key,
			      const char *val)
{
...
static int stack_config_validate(void)
{
	...
	if (kwatch_stack_cfg.watch_len == 0 || kwatch_stack_cfg.watch_len > 8)
		return -ERANGE;

	if (kwatch_stack_cfg.sp_offset > 4096)
		return -EINVAL;
...
static int kwatch_stack_resolve(struct pt_regs *regs,
				ulong *out_addr, u16 *out_len)
{
	...
	len = kwatch_stack_cfg.watch_len ? kwatch_stack_cfg.watch_len : sizeof(ulong);
